...
1. Lis `specs/$ARGUMENTS` (accepte un id `p01-02` ou un nom de fichier). Statut `ready` obligatoire ; sinon arrête-toi et dis pourquoi.
   Si une sheet **existe déjà** au chemin déduit, c'est une **mise à jour** : lis-la d'abord, n'écris que le delta, et suis « Réviser une sheet publiée » dans `CLAUDE.md`.
...
7. Passe le spec au statut `built`. Commit `feat(sheet): <id> <titre>`.
   En mise à jour, la version est **bumpée** (`<title>`, meta `status=vN`, footer), jamais remise à v1, et le commit dit `feat(sheet): <id> révision vN — <trois mots>`.
