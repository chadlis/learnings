...
- **Conventions de λ entre p02-02 et p03-01** — validé 16/09 : p03-01 (habit 3) chiffre ½(θ−1)² + λθ² et obtient θ̂ = 1/(1+2λ) et max(0, 1−λ). p02-02 donne le seuillage doux sous ½(β−b)² + λ·pen(β), pen = ½β² ou |β|, d'où b/(1+λ) et signe(b)·max(0, |b|−λ). Les deux sont exacts mais les λ diffèrent d'un facteur 2 : à harmoniser si les deux fiches sont lues côte à côte.
...
Reste un point à trancher en revue 2, hors du périmètre de celle-ci : le bloc « zéro exact en forme fermée » (pas 4) écrit le critère `½(β − b)² + λ·pen(β)` avec `pen = ½β²`, d'où `b/(1+λ)` et `signe(b)·max(0, |b| − λ)`. La convention de dépôt fixée le 16/09 (point 2 de la revue) est `L + λ·pen` **sans ½**. Sous cette convention le résultat L2 est inchangé — `b/(1+λ)` — mais le seuil L1 devient `max(0, |b| − λ/2)`. p02-02 n'a pas été touchée, la revue l'ayant validée ; l'écart ne porte que sur ce seuil-là.
